/* ══════════════════════════════════════════════════════════════
   둘째 달 페이지
   밤마다 content/second-moon.js 의 steps 를 차례대로 보여 줍니다.
   화면 종류: intro 들어가기, pairs 낱말 카드, choose 고르기(글 또는 그림), rule 규칙,
   josa 이에요/예요 고르기, myname 내 이름, build 문장 만들기, sound 소리와 글자,
   dialogue 대화 듣기, dict 받아쓰기, task 가족 과제. 마지막에 결과 화면이 붙습니다.
   ══════════════════════════════════════════════════════════════ */

/* ---- 진도 저장: {stars, start, updated}. 첫째 달과 같은 모양입니다. ---- */
const STORE_KEY = SECOND_MOON.store;
const OPEN = M2_NIGHTS.map(x => x.n);
function loadStars(){
  const out = {};
  ...
  try {
    ...
    if(d && d.stars && typeof d.stars === 'object'){
      for(const k in d.stars){
        ...
        if(n >= 1 && n <= M2_TOTAL && v >= 1 && v <= 3) out[n] = Math.round(v);
      }
    }
  } catch(e) {}
  return out;
...
function firstMoonDone(){
  ...
  try {
    const d = JSON.parse(localStorage.getItem('daltokki:v1:first-moon') || 'null');
    if(!d) return false;
    const st = d.stars || {}, start = Number(d.start) || 1;
    for(let n = 1; n <= 8; n++) if(!st[n] && n >= start) return false;
    return true;
...
const nightTitle = n => { const x = M2_NIGHTS.find(m => m.n === n); return x ? x.title : ''; };
...
function showPicker(){
    ...
    B.nights.forEach((n, i) => {
      ...
      if(!open){
        row.append(h('div', {class:'modcard locked', 'aria-hidden':'true'},
          h('div', {html: moonSVG(n, 48, false, M2_TOTAL)}),
  ...
  const resetBtn = h('button', {}, '둘째 달 기록 지우기');
  let armed = false;
...
SCREENS.pairs = S => {
  ...
  if(S.pairs){
    ...
    S.pairs.forEach(P => {
      ...
      g.append(h('div', {class:'paircard'},
        h('div', {class:'pic', html: M2_PIC[P.pic] || ''}),
  ...
  if(S.singles){
    ...
    S.singles.forEach(item => {
      ...
      el.innerHTML = item.pic && M2_PIC[item.pic] ? M2_PIC[item.pic] : `<div class="noimg">${item.w}</div>`;
      const en = h('div', {class:'en'}, '');
...
SCREENS.dict = S => {
  ...
  function draw(){
    ...
    box.append(
      row('자음', M2_POOL.cho, j => { pendingCho = j; setFeedback(''); paint(); }),
      row('모음', M2_POOL.jung, j => { if(!pendingCho){ setFeedback('자음을 먼저 눌러요.', 'no'); return; } typed += compose(pendingCho, j); pendingCho = null; setFeedback(''); paint(); }),
      row('받침', M2_POOL.jong, j => { if(!typed){ setFeedback('먼저 글자를 만들어요.', 'no'); return; } typed = typed.slice(0, -1) + addJong(typed[typed.length - 1], j); setFeedback(''); paint(); }));
...
function listClips(){
  ...
  const add = (text, n, label, sayHow) => {
    ...
    seen.add(text);
    out.push({id: clipId(text), text, night: n, month: 2, label, say: sayHow || '자연스러운 속도로 한 번'});
  };
  const VOICE = {tori:'토리 목소리: 밝은 아이처럼', moi:'모이 목소리: 토리 친구처럼 명랑하게', dami:'담이 목소리: 할아버지처럼 느긋하게'};
  M2_NIGHTS.forEach(N => N.steps.forEach(S => {
    const n = N.n;
